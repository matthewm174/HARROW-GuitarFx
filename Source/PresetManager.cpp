...
namespace Service
{
    ...
    int PresetManager::loadNextPreset()
    {
        ...
        const auto nextIndex = currentIndex + 1 > (allPresets.size() - 1) ? 0 : currentIndex + 1;//circular rotation
    ...
    int PresetManager::loadPreviousPreset()
    {
        ...
        const auto currIdx = allPresets.indexOf(currentPreset.toString());
        const auto prevIdx = currIdx - 1 < 0 ? allPresets.size() - 1 : currIdx - 1;//circular rotation
        loadPreset(allPresets.getReference(prevIdx));
        return prevIdx;
